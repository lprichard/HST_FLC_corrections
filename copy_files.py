...
def copy_files_check(src_dir, dst_dir, files='*', rename=False, src_str='', dst_str='', move=False, print_cmd=False):
    ...
    for file in files:
        if os.path.isfile(file):
            ...
            src = os.path.join(src_dir, file)       # Source filepath of file to be copied/moved

            # If a file is to be renamed with a replacement extension
            if rename == False:
                dst = os.path.join(dst_dir, file)   # Destination filepath of file to be copied/moved and renamed
            else: 
